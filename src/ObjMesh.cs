...
namespace Thralls
{
    ...
    internal static class ObjMesh
    {
        public static ModelData Load(string path)
        {
            ...
            {
                ThrallsPlugin.Log.LogError("Could not read " + path + ": " + e.Message);
                return null;
            }
        }
        ...
        private static ModelData Parse(string[] lines, string name)
        {
            ...
            {
                ThrallsPlugin.Log.LogError("Model " + name + " has no geometry.");
                return null;
            }
            ...
            if (outPositions.Count > 65535)
            {
                ...
                ThrallsPlugin.Log.LogInfo(name + " needs 32 bit indices ("
            ...
            var baked = ThrallConfig.AltarBakedShading.Value
                        && outShades.Count == outPositions.Count;

            if (baked) mesh.colors = outShades.ToArray();
            else
            {
            ...
            ThrallsPlugin.Log.LogInfo("Vertex colours for " + name + ": "
                                      + (baked ? "baked occlusion" : "plain white"));
            ...
            ThrallsPlugin.Log.LogInfo(string.Format("Loaded {0}: {1} verts, {2} tris, parts [{3}]",
